...
namespace PowerBIReact.Helpers
{
    public class PbiEmbedService
    {
        ...
        public static string GetEmbedParam(string accessToken, EmbedConfiguration appSettings, string ReportId, String WorkspaceId)
        {
            try
            {
                /*JObject apiResponse = GetEmbedUrl(appSettings.WorkspaceId, appSettings.ReportId, accessToken);*/
                JObject apiResponse = GetEmbedUrl(WorkspaceId, ReportId, accessToken);
                string embedUrl = apiResponse.GetValue("embedUrl").ToString();
                string datasetId = apiResponse.GetValue("datasetId").ToString();

                apiResponse = GetEmbedToken(datasetId, ReportId, accessToken);
